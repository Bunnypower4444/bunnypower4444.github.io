...
function mouseReleased() {
    if (chipToAdd && mouseY < sketch.children.CreateScreen.children.Workspace.actualH) {
        if (chipToAdd instanceof BuiltInChip) {
            let heightOffset = 0;
            for (let i = 0; i < chipToAddCount; i++) {
                let ch = new BuiltInChip(chipToAdd.name, chipToAdd.fn, chipToAdd.numInputs, chipToAdd.numOutputs);
...
class Wire {
    ...
    update(val) {
        ...
        this.value = val;
        if (this.end) addUpdate(() => this.end.update(val));
    }
